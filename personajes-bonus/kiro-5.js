/* Kiro · espía del hoyo — personaje bonus listo para el juego.
   Uso:  <script src="personajes-bonus/kiro-5.js"></script>
         BONUS.montar('kiro-5', document.querySelector('.hoyo'));
   Se dibuja igual que en la galería: trae su propio CSS y se inyecta una sola vez. */
(function(){
  var ID  = "kiro-5";
  var CSS = "@keyframes bn_kiroPeek{0%,6%{transform:translateY(58%)}16%,52%{transform:translateY(-4%)}62%,94%{transform:translateY(58%)}100%{transform:translateY(58%)}}@keyframes bn_peekEyesSide{0%,16%,52%,100%{transform:translateX(0)}26%,34%{transform:translateX(-4px)}42%{transform:translateX(4px)}}.bonus-kiro-5{animation:bn_kiroPeek 6s infinite}.bonus-kiro-5 .p-eyes{animation:bn_peekEyesSide 6s infinite}";
